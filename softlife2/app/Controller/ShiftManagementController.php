...
class ShiftManagementController extends AppController {
    ...
    public function index($date = null) {
        ...
        $user_arr = $this->User->find('list', array('fields'=>array('username', 'name')));
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            // スケジュール削除
                ...
                $id = $id_array[0];
                if (strstr($id, 'u')) {
                    $data = $this->User->find('first', array('conditions'=>array('username'=>ltrim($id, 'u'))));
                    $name = $data['User']['name_sei'].' '.$data['User']['name_mei'];
                } else {
                    $data = $this->StaffMaster->find('first', array('conditions'=>array('id'=>$id)));
                    $name = $data['StaffMaster']['name_sei'].' '.$data['StaffMaster']['name_mei'];
                }
                $conditions3 = array('staff_id' => $id, 'class' => $selected_class, 'work_date >= ' => $month.'01', 'work_date <= ' => $month.'31');
                if ($this->StaffSchedule->deleteAll($conditions3, false)) {
                    $this->Session->setFlash('【情報】'.$name.'さんの'.date('n月', strtotime($month.'01')).'の予定を削除しました。');
                } else {
                    $this->Session->setFlash('【エラー】削除処理中にエラーが発生しました。');
                } 
    ...
    public function schedule_old() {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if (isset($data['assignment'])) {
                ...
                $this->redirect(array('action'=>'schedule_old', '?date='.date('Y-m', strtotime($data['month']))));
            /** 保存、重複チェック、確定 **/
                ...
                $this->redirect(array('action'=>'schedule_old', '?date='.date('Y-m', strtotime($month2))));
            /** シフトの全クリア **/
        ...
        foreach($request_staffs as $key=>$data6) {
            ...
            foreach($datas5 as $col=>$data5) {
                ...
                $result3 = $this->OrderCalendar->find('first', array('conditions'=>$conditions9));
                if ($result3['OrderCalendar']['d'.$j] == 0 || !in_array($shokushu_id, $shokushu_ids)) {
                    $point[$col] = -1;
                    continue;
                }
    ...
    public function schedule_old2() {
        ...
        // 推奨スタッフ
        for ($i=1; $i<=20; $i++) {
            $results = $this->OrderInfo->find('all', array('fields'=>array('id', 'staff_ids'.$i), 'conditions'=>array('class'=>$selected_class)));
            //$this->log($results, LOG_DEBUG);
            foreach ($results as $result) {
                $list_staffs[$result['OrderInfo']['id']][$i] = explode(',', $result['OrderInfo']['staff_ids'.$i]);
                foreach($list_staffs[$result['OrderInfo']['id']][$i] as $j=>$staff_id) {
                    if (empty($staff_id)) {
                        $list_staffs2[$result['OrderInfo']['id']][$i][$j] = '';
                        continue;
                    }
                    //$this->StaffMaster->virtualFields['name'] = 'CONCAT(name_sei, " ", name_mei)';
                    //$list_staffs2[$result['OrderInfo']['id']][$i][$j] = $this->StaffMaster->find('first', array('fields'=>array('id', 'name'), 'conditions'=>array('id'=>$staff_id)));
                    if (strstr($staff_id, 'u')) {
                        $list_staffs2[$result['OrderInfo']['id']][$i][$j] = array('id'=>$staff_id, 'name'=>$user_arr[ltrim($staff_id, 'u')]);
                    } else {
                        $list_staffs2[$result['OrderInfo']['id']][$i][$j] = array('id'=>$staff_id, 'name'=>$staff_arr[$staff_id]);
                    }
        ...
        $this->set('month', ltrim($month, '0'));
        // ポイント更新フラグ
        if (empty($this->request->query('point'))) {
            $flag_point = false;
        } else {
            $flag_point = true;
        }
        ...
        $this->set('datas', $datas);
        $this->log($datas, LOG_DEBUG);
        // カレンダー差し込みカウンタ配列:$cal_arr
        if (empty($datas)) {
            $this->set('cal_arr', null);
        } else {
            $diff = 0;
            $val = -1;
            foreach($datas as $key=>$data) {
                $diff = $data[0]['cnt'];
                $cal_arr[$key] = $val + $diff;
                $val = $cal_arr[$key];
            }
        ...
        foreach($datas2 as $key=>$data) {
            ...
            // 推奨スタッフ
            if (!empty($list_staffs2[$data['OrderInfoDetail']['order_id']][$data['OrderInfoDetail']['shokushu_num']])) {
                foreach ($list_staffs2[$data['OrderInfoDetail']['order_id']][$data['OrderInfoDetail']['shokushu_num']] as $key2=>$value) {
                    if (empty($value['id'])) {
                        $value['id'] = '';
                    }
     ...
     */
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if (isset($data['assignment'])) {
                ...
                $conditions6 = array(
                    'StaffSchedule.class'=>$selected_class, 
                    'work_date >= '=>$year.'-'.sprintf('%02d',$month).'-01',
                    'work_date <= '=>$year.'-'.sprintf('%02d',$month).'-31',
                    'OR' => array(array('work_flag'=>1), array('work_flag'=>2))
                    );
                $request_staffs = $this->StaffSchedule->find('all', array('conditions'=>$conditions6, 'order'=>array('staff_id')));
                $this->log($this->StaffSchedule->getDataSource()->getLog(), LOG_DEBUG);
                if (empty($request_staffs)) {
                    $this->Session->setFlash('【情報】スタッフからのシフト希望がないため、割付できません。');
                    $this->redirect(array('?date='.$this->request->query('date')));
                    return;
                }
                /** 新しいシフトロジック **/
                ...
                for ($d=1; $d<=31; $d++) {
                    ...
                    foreach($datas_ss as $data_ss) {
                        ...
                        if ($point_max > 0) {
                            ...
                            if (strstr($data_ss['StaffSchedule']['staff_id'], 'u')) {
                                $unasigned[$d][0][] = $data_ss['StaffSchedule']['staff_id'];
                                $count = $count+1;
                                continue;
                            }
                            if (in_array(2, explode(',', $staff_shokushu_arr[$data_ss['StaffSchedule']['staff_id']]))) {
                                $unasigned[$d][0][] = $data_ss['StaffSchedule']['staff_id'];
                                $count = $count+1;
                            } elseif (in_array(9, explode(',', $staff_shokushu_arr[$data_ss['StaffSchedule']['staff_id']]))) {
                                $unasigned[$d][3][] = $data_ss['StaffSchedule']['staff_id'];
                            } else {
                                $unasigned[$d][4][] = $data_ss['StaffSchedule']['staff_id'];
                            }
                        }
                    }
                    for($k=0; $k<$count; $k++) {
                        if ($k%2 == 0) {
                            $unasigned[$d][1][] = $unasigned[$d][0][$k];
                        } else {
                            $unasigned[$d][2][] = $unasigned[$d][0][$k];
                        }
                    }
                    // 条件付き
                    $conditions11 = array(
                    ...
                    $datas_ss2 = $this->StaffSchedule->find('all', array('conditions'=>$conditions11));
                    foreach($datas_ss2 as $data_ss2) {
                        $unasigned[$d][5][] = $data_ss2['StaffSchedule']['staff_id'];
                    }
                }
                $this->set('worktable_arr', $worktable_arr);
                $this->set('unasigned', $unasigned);
                ...
                for ($i=1; $i<=$col; $i++) {
                    for ($d=1; $d<=31; $d++) {
                        ...
                        if ($i <= $col-5) {
                            ...
                            if (empty($unasigned[$d][1])) {
                                $data2[$i]['d'.$d] = '';
                            } else {
                                $data2[$i]['d'.$d] = implode(',', $unasigned[$d][1]);
                            }
                        } elseif ($i == $col-3) {
                            if (empty($unasigned[$d][2])) {
                                $data2[$i]['d'.$d] = '';
                            } else {
                                $data2[$i]['d'.$d] = implode(',', $unasigned[$d][2]);
                            }
                        } elseif ($i == $col-2) {
                            if (empty($unasigned[$d][3])) {
                                $data2[$i]['d'.$d] = '';
                            } else {
                                $data2[$i]['d'.$d] = implode(',', $unasigned[$d][3]);
                            }
                        } elseif ($i == $col-1) {
                            if (empty($unasigned[$d][4])) {
                                $data2[$i]['d'.$d] = '';
                            } else {
                                $data2[$i]['d'.$d] = implode(',', $unasigned[$d][4]);
                            }
                        } elseif ($i == $col) {
                            if (empty($unasigned[$d][5])) {
                                $data2[$i]['d'.$d] = '';
                            } else {
                                $data2[$i]['d'.$d] = implode(',', $unasigned[$d][5]);
                            }
                        }
                        $data2[$i]['case_id'] = $data['WorkTable'][$i]['case_id'];
                        $data2[$i]['order_id'] = $data['WorkTable'][$i]['order_id'];
            ...
            /** 保存、重複チェック、確定 **/
                ...
                // データがなければ処理停止
                if (empty($data2)) {
                    $this->redirect(array('action'=>'schedule_old2', '?date='.date('Y-m', strtotime($data['month']))));
                    return;
                }
                // 該当月の更新
                if ($data['flag'] == 0) {
                ...
                if ($mode == 2 || $mode == 3) {
                    ...
                    for ($d=1; $d<=31; $d++) {
                        ...
                        if ($this->array_isunique($check_arr) != false) {
                            ...
                            $this->log($check_arr, LOG_DEBUG);
                            if (empty($data9)) {
                                $this->Session->setFlash('【エラー】処理中にエラーが発生しました。('.key($this->array_isunique($check_arr)).')');
                                $this->redirect(array('action'=>'schedule_old2', '?date='.date('Y-m', strtotime($data['month']))));
                                return;
                            }
                            $this->Session->setFlash('【エラー】'.date('n', strtotime($data['month'])).'/'.$d.'に'.$data9['StaffMaster']['name_sei'].$data9['StaffMaster']['name_mei'].'さんが重複しています。'.$commet2);
                    ...
                    if ($mode == 2) {
                        $this->Session->setFlash('【情報】エラーはありません。');
                        // ログ書き込み
                        $this->setShiftLog($username, $selected_class, '', 12);
                    } elseif ($mode == 3) {
                        if ($data['flag'] == 0) {
                ...
                //$this->redirect(array('action'=>'schedule_old2', '?date='.date('Y-m', strtotime($month2))));
            /** シフトの全クリア **/
            } elseif (isset($this->request->data['all_clear'])) {
                // 該当月を削除
                $param = array('class' => $selected_class, 'month' => $data['month'].'-01');
         ...
         */
            ...
            if (!empty($staff_cell) && $staff_cell[0][0] == $year.'-'.$month) {
                ...
                // 未定枠
                for($j=1; $j<=5; $j++) {
                    $conditions1 = array('class'=>$selected_class, 'month'=>$year.'-'.$month.'-01', 'order_id'=>0, 'shokushu_num'=>$j);
                    $results= $this->WorkTable->find('first', array('conditions'=>$conditions1));
                    //$this->log($results, LOG_DEBUG);
                    if (!empty($results)) {
                        $i = $results['WorkTable']['column'];
                        for($d=1; $d<=31; $d++) {
                            $staff_ids[$d][$i] = $results['WorkTable']['d'.$d];
                            if (!empty($results['WorkTable']['d'.$d])) {
                                $staff_ids2[$d][$i] = explode(',', $results['WorkTable']['d'.$d]);
            ...
            if (empty($staff_cell)) {
                ...
                for($d=1; $d<=31; $d++) {
                    for($i=1; $i<=1000; $i++) {
                        ...
                        foreach ($staff_cell[$d][$i] as $key=>$staff_id) {
                            if (strstr($staff_id, 'u')) {
                                $data_staffs[$d][$i][$key] = array('id'=>$staff_id, 'name'=>$user_arr[ltrim($staff_id, 'u')]);
                            } else {
                                $data_staffs[$d][$i][$key] = array('id'=>$staff_id, 'name'=>$staff_arr[$staff_id]);
                            }
                            /**
                            $conditions1 = array('id'=>$staff_id);
                            $data_staffs[$d][$i][$key] = $this->StaffMaster->find('first', array('fields'=>array('id', 'name_sei', 'name_mei'), 'conditions'=>$conditions1));
     ...
     * 稼働表作成（改良版２）
     */
    public function schedule($para = null, $case_id = null) {
        // レイアウト関係
        $this->layout = "main";
        $this->set("title_for_layout", $this->title_for_layout);
        // タブの状態
        ...
        $this->set('page', $page);
        if (empty($this->params['named']['hidden'])) {
            $hidden = '0';
        } else {
            $hidden = $this->params['named']['hidden'];
        }
        $this->set('hidden', $hidden);
        //$this->log('hidden='.$hidden, LOG_DEBUG);
        // 案件名の取得
        $conditions1 = array('class'=>$selected_class, 'kaijo_flag'=>0);
        $getCasename = $this->CaseManagement->find('list', array('fields'=>array('id', 'case_name'), 'conditions' => $conditions1, 'order' => array('sequence')));
        $this->set('getCasename', $getCasename);
        ...
        $this->set('list_distributor', $datas11);
        // 依頼主
        $result12 = $this->CaseManagement->find('all', array(
            'fields'=>array('id', 'client'), 'conditions' => $conditions1, 'order' => array('sequence')));
        //$this->log($result1, LOG_DEBUG);
        $datas22 = null;
        ...
        $this->set('staff_arr', $staff_arr);
        // 紹介可能職種
        $datas12 = $this->StaffMaster->find('all', array('fields'=>array('id', 'shokushu_shoukai')));
        foreach($datas12 as $data12) {
            $shokushu_arr0[$data12['StaffMaster']['id']] = $data12['StaffMaster']['shokushu_shoukai'];
        }
        $this->set('shokushu_arr0', $shokushu_arr0);
        // 社員配列
        $this->User->virtualFields['name'] = 'CONCAT(name_sei, " ", name_mei)';
        $user_arr = $this->User->find('list', array('fields'=>array('username', 'name')));
        $this->set('user_arr', $user_arr);
        ...
        $this->set('year', $year);
        $this->set('month', ltrim($month, '0'));
        //$this->log('$year='.$year.', $month='.$month, LOG_DEBUG);
        // ポイント更新フラグ
        if (empty($this->request->query('point'))) {
            $flag_point = 0;
        } else {
        ...
        $this->set('flag_point', $flag_point);
        //$this->log($flag_point, LOG_DEBUG);
        // 該当月のフラグ
        $conditions32 = array('class'=>$selected_class, 'month'=>$year.'-'.$month.'-01');
        $data = $this->WorkTable->find('first', array('conditions'=>$conditions32));
        if (empty($data)) {
            $flag = 0;
            $this->set('flag', $flag);
        } else {
        ...
        // 登録データのセット
        //$conditions1 = array('id' => $order_id, 'case_id' => $case_id);
        $conditions6 = array('class'=>$selected_class, 'OrderCalendar.year' => $year, 'OrderCalendar.month' => $month);
        $col = $this->OrderCalendar->find('count', array('conditions' => $conditions6));
        $this->set('col', $col);
        // 案件あたりの職種数
        /**
        //$this->StaffMaster->virtualFields['name'] = 'CONCAT(name_sei, " ", name_mei)';
        $datas = $this->OrderCalendar->find('all', array('fields'=>array('*', 'count(case_id) as cnt'), 
            'conditions' => $conditions6, 'group' => array('case_id'), 'order' => array('sequence', 'case_id', 'order_id')));
         * 
         */
        //$this->set('datas', $datas);
        /***/
        if (!empty($case_id)) {
            $conditions62 = $conditions6 + array('case_id'=>$case_id);
        } else {
            $conditions62 = $conditions6;
        }
        $this->paginate = array(
            'limit' => $limit,
            'fields' => array('*', 'count(case_id) as cnt'),
            'group' => array('case_id'),
            'conditions' => $conditions62,
            //'order' => array('sequence', 'case_id', 'order_id')
        );
        $order = array(
                'sequence' => 'asc',
                'case_id' => 'asc'
            );
        $this->OrderCalendar->setOrder($order);
        $datas = $this->paginate('OrderCalendar');
        $this->set('datas', $datas);
        $this->log($datas, LOG_DEBUG);
        // 案件リスト
        $case_arr0 = $this->CaseManagement->find('list', array('fields'=>array('id', 'case_name'), 'conditions'=>array('kaijo_flag'=>0, 'class'=>$selected_class)));
        $datas10 = $this->OrderCalendar->find('all', array('fields'=>'case_id', 'group'=>array('case_id'), 
            'conditions' => $conditions6, 'order'=>array('sequence' =>'asc','case_id'=>'asc')));
        $case_arr = null;
        foreach($datas10 as $data10) {
            $case_arr[$data10['OrderCalendar']['case_id']] = $case_arr0[$data10['OrderCalendar']['case_id']];
        }
        ...
        foreach($datas2 as $data2) {
            if ($data2['OrderInfoDetail']['case_id'] == $datas[0]['OrderCalendar']['case_id']) {
                break;
            }
            $start_count += 1;
        }
        $this->set('start_count', $start_count);
        //$this->log($start_count, LOG_DEBUG);
     ...
     */
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if (isset($data['assignment'])) {
                ...
                $request_staffs2 = $this->StaffSchedule->find('all', array('conditions'=>$conditions62, 'order'=>array('point', 'staff_id')));
                //$this->log($request_staffs, LOG_DEBUG);
                for($i=0; $i<$record; $i++) {
                    $conditions7 = array(
                        'class'=>$selected_class, 
                ...
                //$this->log($datas5, LOG_DEBUG);
                foreach($request_staffs2 as $key=>$data6) {
                    $id = $data6['StaffSchedule']['id'];
                    $staff_id = $data6['StaffSchedule']['staff_id'];
                    $work_date = $data6['StaffSchedule']['work_date'];
                    if (strstr($staff_id, 'u')) {
                        ...
                        $shokushu_ids = explode(',', $shokushu_arr0[$data6['StaffSchedule']['staff_id']]);
                    }
                    //$this->log($shokushu_ids, LOG_DEBUG);
                    $y = date('Y', strtotime($work_date));
                    $n = date('n', strtotime($work_date));
                ...
                for ($d=1; $d<=31; $d++) {
                    ...
                    foreach($datas_ss as $data_ss) {
                        $point_max = max(explode(',', $data_ss['StaffSchedule']['point']));
                        if ($point_max > 0) {
                            $column = array_search($point_max, explode(',', $data_ss['StaffSchedule']['point']));
                            $worktable_arr[$d][$column][] = $data_ss['StaffSchedule']['staff_id'];
                        } else {
                            if (strstr($data_ss['StaffSchedule']['staff_id'], 'u')) {
                                $unasigned[$d][0][] = $data_ss['StaffSchedule']['staff_id'];
                            ...
                            if (in_array(2, explode(',', $shokushu_arr0[$data_ss['StaffSchedule']['staff_id']]))) {
                                $unasigned[$d][0][] = $data_ss['StaffSchedule']['staff_id'];
                                $count = $count+1;
                            } elseif (in_array(9, explode(',', $shokushu_arr0[$data_ss['StaffSchedule']['staff_id']]))) {
                                $unasigned[$d][3][] = $data_ss['StaffSchedule']['staff_id'];
                            } else {
                                $unasigned[$d][4][] = $data_ss['StaffSchedule']['staff_id'];
                            }
                ...
                for ($i=0; $i<$col+5; $i++) {
                    for ($d=1; $d<=31; $d++) {
                        ...
                        if ($i < $col) {
                            ...
                            $data2[$i]['case_id'] = $datas2[$i]['OrderInfoDetail']['case_id'];
                            $data2[$i]['order_id'] = $datas2[$i]['OrderInfoDetail']['order_id'];
                            $data2[$i]['shokushu_num'] = $datas2[$i]['OrderInfoDetail']['shokushu_num'];
                            $data2[$i]['shokushu_id'] = $datas2[$i]['OrderInfoDetail']['shokushu_id'];
                        } elseif ($i == $col) {
                            if (empty($unasigned[$d][1])) {
                                $data2[$i]['d'.$d] = '';
                            } else {
                            ...
                            $data2[$i]['case_id'] = 0;
                            $data2[$i]['order_id'] = 0;
                            $data2[$i]['shokushu_num'] = 1;
                            $data2[$i]['shokushu_id'] = 2;
                        } elseif ($i == $col+1) {
                            if (empty($unasigned[$d][2])) {
                                $data2[$i]['d'.$d] = '';
                            } else {
                            ...
                            $data2[$i]['case_id'] = 0;
                            $data2[$i]['order_id'] = 0;
                            $data2[$i]['shokushu_num'] = 3;
                            $data2[$i]['shokushu_id'] = 9;
                        } elseif ($i == $col+3) {
                            if (empty($unasigned[$d][4])) {
                                $data2[$i]['d'.$d] = '';
                            } else {
                ...
                $this->redirect(array('action'=>'schedule', date('Y-m', strtotime($this->request->data['month'])), 'limit'=>$this->request->data['limit'], 'page'=>$page, 'hidden'=>$hidden));
            /** 保存、チェック、確定 **/
            } elseif (isset($data['mode'])) {
                $data2 = null;
                $mode = $data['mode'];
                $month2 = $data['month'];
                $limit2 = $data['limit'];
                $count_from = $data['count_from']+1;
                //$col = $data['col'];
                $col = count($data['WorkTable']);
                for ($i=$count_from; $i<=$count_from+$col-1; $i++) {
                    if ($i == $count_from+$col-5) {
                        $column = $data['col']+1;
                    } elseif ($i == $count_from+$col-4) {
                        $column = $data['col']+2;
                    } elseif ($i == $count_from+$col-3) {
                        $column = $data['col']+3;
                    } elseif ($i == $count_from+$col-2) {
                        $column = $data['col']+4;
                    } elseif ($i == $count_from+$col-1) {
                        $column = $data['col']+5;
                    } else {
                        $column = $i;
                    }
                    //$this->log('$i='.$i.', $column='.$column, LOG_DEBUG);
                    for ($d=1; $d<=31; $d++) {
                        if (empty($data[$d.'_'.$column])) {
                            $data_d = null;
                            //continue;
                        } else {
                            $data_d = $data[$d.'_'.$column];
                        }
                        $data2[$i]['column'] = $column;
                        $data2[$i]['class'] = $selected_class;
                        $data2[$i]['username'] = $username;
                        $data2[$i]['month'] = '"'.date('Y-m-d', strtotime($data['month'])).'"';
                        $data2[$i]['d'.$d] = '"'.$data_d.'"';
                        $data2[$i]['case_id'] = $data['WorkTable'][$column]['case_id'];
                        $data2[$i]['order_id'] = $data['WorkTable'][$column]['order_id'];
                        $data2[$i]['shokushu_num'] = $data['WorkTable'][$column]['shokushu_num'];
                        $data2[$i]['shokushu_id'] = $data['WorkTable'][$column]['shokushu_id'];
                    }
                }
                // データがなければ処理停止
                if (empty($data2)) {
                ...
                if ($data['flag'] == 0) {
                    for ($i=$count_from; $i<=$count_from+$col-1; $i++) {
                        ...
                        if ($this->WorkTable->updateAll($data2[$i], $conditions)) {
                            //$this->log('go:'.$i, LOG_DEBUG);
                            //$this->log($this->WorkTable->getDataSource()->getLog(), LOG_DEBUG);
                        } else {
                            //$this->log('stop:'.$i, LOG_DEBUG);
                            break;
                        }
                    }
                }
                ...
                if ($mode == 2 || $mode == 3) {
                    ...
                    $conditions7 = array(
                        ...
                        'month' => date('n', strtotime($month2))
                    );
                    $results = $this->OrderCalendar->find('all', array('conditions'=>$conditions7));
                    foreach($results as $value) {
                        $order_id = $value['OrderCalendar']['order_id'];
                        $shokushu_num = $value['OrderCalendar']['shokushu_num'];
            ...
            /** シフトの全クリア **/
                ...
                $this->redirect(array('action'=>'schedule', date('Y-m', strtotime($data['month'])), 'limit'=>$data['limit'], 'page'=>$page, 'hidden'=>$hidden));
            // 案件絞込
            } elseif (isset($this->request->data['select_case'])) {
                $case_id = $this->request->data['select_case'];
                if (!empty($case_id)) {
                    $this->redirect(array('action'=>'schedule', date('Y-m', strtotime($data['month'])), $case_id, 'limit'=>$data['limit'], 'hidden'=>$hidden));
                }
            // 所属の変更
            } elseif (isset($this->request->data['class'])) {
                $this->selected_class = $this->request->data['class'];
            ...
            // 表示件数の変更
                ...
                $limit = $this->request->data['limit'];
                $this->set('limit', $limit);
                //$this->log(str_replace('-', '', $this->request->data['month']), LOG_DEBUG);
                $this->redirect(array('limit' => $limit, 'hidden' => $hidden, $data['month']));
            } else {
         ...
         */
            ...
            if ($flag_point == 1) { 
                ...
                // 表示の更新
                $this->redirect(array('action'=>'schedule', $year.'-'.sprintf('%02d',$month), 'limit'=>$limit, 'page' => $page, 'hidden'=>$hidden)); 
            }
        } else {
     ...
     * 稼働表作成（編集画面）
     */
    public function schedule_edit($case_id = null) {
        // レイアウト関係
        $this->layout = "sub";
        $this->set("title_for_layout", $this->title_for_layout);
        // タブの状態
        ...
        // 登録データのセット
        //$conditions1 = array('id' => $order_id, 'case_id' => $case_id);
        $conditions1 = array('class'=>$selected_class, 'OrderCalendar.year' => $year, 'OrderCalendar.month' => $month, 'OrderCalendar.case_id' => $case_id);
        $col = $this->OrderCalendar->find('count', array('conditions' => $conditions1));
        $this->set('col', $col);
        // 案件あたりの職種数
        //$this->StaffMaster->virtualFields['name'] = 'CONCAT(name_sei, " ", name_mei)';
        ...
        //$this->log($this->OrderCalendar->getDataSource()->getlog(), LOG_DEBUG);
        // 職種以下
        $or_arr = array(array('OrderCalendar.case_id' => $case_id), array('OrderCalendar.case_id' => 0));
        $option = array();
        $option['fields'] = array('OrderInfoDetail.*', 'OrderCalendar.*'); 
        $option['order'] = array('OrderCalendar.sequence' => 'asc', 'OrderInfoDetail.case_id' => 'asc', 'OrderInfoDetail.order_id' => 'asc', 'OrderInfoDetail.shokushu_num' => 'asc');
        $option['conditions'] = array('OrderInfoDetail.class'=>$selected_class, 'OrderCalendar.year' => $year, 'OrderCalendar.month' => $month, 'OR' => $or_arr); 
        $option['joins'] = array(
        array(
            'type' => 'RIGHT',   //LEFT, INNER, OUTER
            'table' => 'order_calendars',
     ...
     * 帳票出力
     */
    public function report($case_id = null, $date = null) {
        // レイアウト関係
        $this->layout = "main";
        $this->set("title_for_layout", $this->title_for_layout);
        // タブの状態
    ...
    public function setting($id = null, $id2 = null, $row =null, $direction = null) {
        ...
        // レコード数
        $option = array(
            'conditions' => array('class' => $selected_class, 'kaijo_flag' => 0),
            'limit' => '30',
    ...
    public function input_schedule($staff_id = null) {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            // 選択
                ...
                $this->set('datas2', $datas2);
                //$this->log($datas2, LOG_DEBUG);
                $this->redirect(array('action'=>'input_schedule', $id, '?date='.$date));
            // 消去
            } elseif (isset($this->request->data['erasure'])) {
                // セッション削除
                $this->Session->delete('schedule_staff');
                $this->redirect(array('action'=>'input_schedule', '?date='.$date));
            // 登録
            } elseif (isset($this->request->data['commit'])) {
                if (empty($staff_id)) {
                    $this->Session->setFlash('【エラー】スタッフを選択してください。');
                    return;
                }
                if ($this->StaffSchedule->saveAll($this->request->data['StaffSchedule'])) {
                    $this->StaffMaster->virtualFields['name'] = 'CONCAT(name_sei, " ", name_mei)';
     ...
     * @param type $shokushu_num
     */
    public function input_schedule2($uid = null) {
        // レイアウト関係
        $this->layout = "sub";
        $this->set("title_for_layout", $this->title_for_layout);
        // 初期セット
        ...
        $this->set('datas1', null);
        if (empty($uid)) {
            $this->Session->delete('schedule_user');
        }
        $this->set('uid', $uid);
        $datas2 = null;
        //$this->Session->write('staff_cell', $staff_cell);
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if (isset($this->request->data['select'])) {
                $id = $this->request->data['StaffSchedule']['select_user'];
                // 選択は一つだけ
                $uid = $this->Session->read('schedule_user');
                if (count($uid) == 1) {
                    $this->Session->setFlash('【エラー】すでにスタッフが選択されています。('.$uid.')');
                    $this->redirect(array('action' => 'input_schedule2', $uid ));
                    return;
                }
                // セッション格納
                $this->Session->write('schedule_user', $id);
                // 選択中スタッフ
                $this->User->virtualFields['name'] = 'CONCAT(name_sei, " ", name_mei)';
                $data2 = $this->User->find('first', array('fields'=>array('*', 'name'), 'conditions'=>array('username'=>$id)));
                $this->set('data2', $data2);
                //$this->log($datas2, LOG_DEBUG);
                $this->redirect(array('action'=>'input_schedule2', $id, '?date='.$date));
            // 消去
            } elseif (isset($this->request->data['erasure'])) {
                // セッション削除
                $this->Session->delete('schedule_user');
                $this->redirect(array('action'=>'input_schedule2', '?date='.$date));
            // 登録
            } elseif (isset($this->request->data['commit'])) {
                if (empty($uid)) {
                    $this->Session->setFlash('【エラー】スタッフを選択してください。');
                ...
                if ($this->StaffSchedule->saveAll($this->request->data['StaffSchedule'])) {
                    $this->User->virtualFields['name'] = 'CONCAT(name_sei, " ", name_mei)';
                    $datas = $this->User->find('first', array('fields'=>array('*', 'name'), 'conditions'=>array('username'=>$uid)));
                    $this->log($datas, LOG_DEBUG);
                    // セッションの削除
                    $this->Session->delete('schedule_staff');
                    // ログ書き込み
                    $this->setShiftLog($username, $selected_class, 
                            'シフト希望：'.$datas['User']['name'].'(u'.$uid.') '.  str_replace('-', '年', $date).'月分', 1);
                    $this->Session->setFlash('【情報】シフト希望を登録いたしました。');
                    $this->redirect(array('action'=>'input_schedule2', $uid, '?date='.$date));
                }
            } else {
        ...
        // 登録済みデータ（社員）
        $this->User->virtualFields['name'] = 'CONCAT(name_sei, " ", name_mei)';
        $data2 = $this->User->find('first', array('fields'=>array('*', 'name'), 'conditions'=>array('username'=>$uid)));
        $this->set('data2', $data2);
        // 登録済みデータ（シフト希望）
        $conditions3 = array('staff_id'=>'u'.$uid, 'class'=>$selected_class, 'work_date >='=>$year.'-'.$month.'-01', 'work_date <='=>$year.'-'.$month.'-31');
        $datas3 = $this->StaffSchedule->find('all', array('conditions'=>$conditions3));
        $this->set('datas3', $datas3);
        $this->log($this->StaffSchedule->getDataSource()->getLog(), LOG_DEBUG);
        $this->log($datas3, LOG_DEBUG);
    ...
    public function select($order_id = null, $col = null, $cell_row = null, $cell_col = null, $shokushu_num = null) {
        ...
        // スタッフの取得
        for($i=1; $i<=20; $i++) {
            if (isset($this->request->query['s'.$i])) {
                $staff_ids[] = $this->request->query['s'.$i];
            }
        }
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            // 待ち合わせ登録
                ...
                if ($this->WorkTable->updateAll($data, $conditions)) {
                    $this->Session->setFlash('【情報】'.$comment);
                    foreach($staff_ids as $i=>$staff_id) {
                        $sid = '&s'.($i+1).'='.$staff_id;
                    }
                    $this->redirect(array('action'=>'select', $order_id, $col, $cell_row, $cell_col, $shokushu_num, '?date='.$month.$sid));
                }
            // 消去
            } elseif (isset($this->request->data['erasure'])) {
        ...
        $this->log($shokushu_id, LOG_DEBUG);
        $conditions6 = array(
            'StaffSchedule.class'=>$selected_class,
        ...
        if (!empty($staff_ids)) {
            ...
            foreach($staff_ids as $key=>$staff_id) {
                ...
                $datas22[$key] = $this->StaffSchedule->find('first', array('fields'=>array('*'), 'conditions'=>$conditions7 + array('staff_id'=>$staff_id))); 
            }
    ...
    // スタッフの選択（小画面）
    public function select2($order_id = null, $col = null, $cell_row = null, $cell_col = null) {
        // レイアウト関係
        $this->layout = "sub";
        $this->set("title_for_layout", $this->title_for_layout);
        // 初期セット
        ...
        $this->set('page', $page);
        if (!empty($this->request->query('hidden'))) {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            // 選択（スタッフ）
                ...
                if (count($session_id) == 15) {
                    ...
                    $this->redirect(array('action' => 'select2', $order_id ,$col));
                    return;
                }
                // セッション格納
                if (empty($session_id)) {
    ...
    function array_isunique($array){
            ...
            if(count($arrayValue)!=0){
                foreach($arrayValue as $key=>$value) {
                    if (count($arrayValue[$key]) > 2) {
                        $arrayValue2[] = $arrayValue[$key];
                    }
                }
                return $arrayValue2;
            }else{
