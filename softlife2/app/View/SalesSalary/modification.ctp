...
<div id='headline' style="padding:10px 10px 10px 10px;">
    ...
    &nbsp;
    <b><font Style="font-size:95%;color: yellow;">[個人情報変更]</font></b>
    &nbsp;
</div>
<!-- 見出し１ END -->
...
?>
    <div style="float:left;margin-left: 10px;margin-top: 5px;">
        <?php if ($flag == 1) { ?>
        【表示】<b><font style="background-color: yellow;color:red;">変更あり</font></b> | <a href="<?=ROOTDIR ?>/SalesSalary/modification/0" style="color:black;">更新なし</a>
        <?php } elseif ($flag == 0) { ?>
        【表示】<a href="<?=ROOTDIR ?>/SalesSalary/modification/1" style="color:red;">変更あり</a> | <b><font style="background-color: yellow;color:black;">更新なし</font></b>
        <?php } ?>
    </div>
...
<table id="staff_master" border="1" width="100%" cellspacing="0" cellpadding="5" bordercolor="#333333" align="center" style="font-size: 90%;margin: 0px 0px 5px 0px;">
  ...
  <tr style="font-size: 100%;margin-top: -10px;">
    <th rowspan="2" style="width:5%;"><?php echo $this->Paginator->sort('id',"登録番号");?></th>
    <th rowspan="2" style="width:10%;"><?php echo $this->Paginator->sort('name_sei','氏名', array('escape' => false));?></th>
    <th rowspan="2" colspan="1" style="width:10%;color:white;">変更種別</th>
    <th rowspan="1" colspan="5" style="width:60%;color:white;">連絡先</th>
    <th rowspan="2" colspan="1" style="width:15%;"><?php echo $this->Paginator->sort('modified','更新日時', array('escape' => false));?></th>
  </tr>
  <tr>
    <th style="width:5%;"><?php echo $this->Paginator->sort('name_kouza_reg','処理済', array('escape' => false));?></th>
    <th style="width:10%;"><?php echo $this->Paginator->sort('bank_name','電話番号（携帯）', array('escape' => false));?></th>
    <th style="width:10%;"><?php echo $this->Paginator->sort('name_shiten','電話番号２', array('escape' => false));?></th>
    <th style="width:20%;"><?php echo $this->Paginator->sort('bank_name','メールアドレス（携帯）', array('escape' => false));?></th>
    <th style="width:20%;"><?php echo $this->Paginator->sort('name_shiten','メールアドレス２', array('escape' => false));?></th>
  </tr>
  <tr>
  ...
  <?php foreach ($datas as $i=>$data): ?>
    <?php
        if ($data['StaffMaster']['koushin_flag2'] == 1 && $data['StaffMaster']['koushin_flag4'] == 1) {
            $type = '連絡先＆パスワード';
            $bgcolor = '#FFFFCC';
        } elseif ($data['StaffMaster']['koushin_flag2'] == 1) {
            $type = '連絡先';
            $bgcolor = '#FFFFCC';
        } elseif ($data['StaffMaster']['koushin_flag4'] == 1) {
            $type = 'パスワード';
            $bgcolor = '#FFFFCC';
        } else {
            $type = '';
...
<tr style="background-color:<?=$bgcolor?>;">
    ...
    <td align="center">
        ...
        <input type="hidden" name="data[StaffMaster][<?=$i?>][koushin_flag2]" value="0">
        <input type="hidden" name="data[StaffMaster][<?=$i?>][koushin_flag4]" value="0">
    </td>
    <!-- 氏名 -->
    ...
    <!-- 電話番号１ -->
    <td align="center" style="font-size: 100%;">
        <?php echo $data['StaffMaster']['telno1']; ?>
    </td>
    <!-- 電話番号２ -->
    <td align="left" style="font-size: 100%;">
        <?php echo $data['StaffMaster']['telno2']; ?>
    </td>
    <!-- メールアドレス１ -->
    <td align="center" style="font-size: 100%;">
        <?php echo $data['StaffMaster']['email1']; ?>
    </td>
    <!-- メールアドレス２ -->
    <td align="left" style="font-size: 100%;">
        <?php echo $data['StaffMaster']['email2']; ?>
    </td>
    <!-- 更新日時 -->
    <td align="center" style="font-size: 100%;">
