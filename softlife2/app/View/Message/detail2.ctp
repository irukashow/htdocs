    ...
    <table id="" border="0" width="99.5%" cellspacing="0" cellpadding="5" bordercolor="#333333" align="center" style="font-size: 90%;">
        ...
        <tr style="background-color: #ffffea;">
            <td>
                ...
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <?= date('Y-m-d', strtotime($data['Message2Staff']['created'])) ?>
...
<?php 
    if (!empty($data['Message2Staff']['attachment'])) {
        ...
        foreach ($files as $file) {
            if (!empty($file)) {
                echo '<a href="'.ROOTDIR.'/files/message/staff/'.sprintf("%010d", $data['Message2Staff']['recipient_staff']).'/'.$file.'" target="_blank">'.$file.'</a><br>';
            }
        }
    } 
?>
...
<br>
<?php echo $this->Html->link('◀送信済みに戻る', 'index/send', array('style'=>'text-decoration: none;')) ?>