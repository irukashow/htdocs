...
onload = function() {
    ...
    var width = <?=50+$col*30+count($datas)*50+$width_extra+30 ?>;
    if (width > 1300) {
        document.body.style.width = '<?=60+$col*30+count($datas)*50+20 ?>px';
    } else {
...
function setCalendar(year, month) {
    ...
    location.href="<?=ROOTDIR ?>/ShiftManagement/schedule_new?date=" +value1 + "-" + value2;
}
// 職種の詳細を隠す
function setHidden() {
