...
onload = function() {
    ...
    document.getElementById("headline").style.width = '1300px';
    document.getElementById("menu_table").style.width = '1300px';
    // スクロールバーを消す
...
document.onkeydown = function (e){
	...
	// 出力テスト
	alert(e.keyCode);
        location.href = "<?=ROOTDIR ?>/ShiftManagement/schedule/limit:<?=$limit ?>/page:";
};
**/
// リロード関数
function doReload() {
    window.location.href = "<?=ROOTDIR ?>/ShiftManagement/schedule/limit:<?=$limit ?>/page:<?=$page ?>/hidden:<?=$hidden ?>?date=<?=$year.'-'.$month ?>&point=true";
    $.blockUI({
...
function setCalendar(year, month) {
    ...
    location.href="<?=ROOTDIR ?>/ShiftManagement/schedule/limit:<?=$limit ?>/hidden:<?=$hidden ?>?date=" +value1 + "-" + value2;
}
// スタッフ選択ページを開く
...
function doHidden() {
    ...
    location.href = "<?=ROOTDIR ?>/ShiftManagement/schedule/limit:<?=$limit ?>/page:<?=$page ?>/hidden:"+hidden+"?date=<?=$year.'-'.sprintf('%02d',$month) ?>";
}
...
function doAccount(year, month, mode) {
    ...
    form.setAttribute('action', '<?=ROOTDIR ?>/ShiftManagement/schedule/hidden:<?=$hidden ?>');
